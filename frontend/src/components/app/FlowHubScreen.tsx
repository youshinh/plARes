...
import type { ProfileInfo, RouteStatus, RouteStepKey, UiText } from '../../types/app';
...
export const FlowHubScreen: FC<FlowHubScreenProps> = ({
  ...
  const [isCompletedRouteExpanded, setIsCompletedRouteExpanded] = useState(!isRouteComplete);
    ...
    <section className="flow-hub hud-animate" aria-label={t.flowHubTitle}>
      ...
      <div className="flow-route-guide">
        <div className="flow-route-summary">
          ...
          {isRouteComplete && (
            ...
            >
              {isCompletedRouteExpanded ? t.routeHideCompleted : t.routeOpenCompleted}
            </button>
          )}
        </div>
      ...
      <div className="flow-hub-grid">
              ...
              {memoryView.entries.length > 1 && (
                <div className="memory-summary-list">
                  {memoryView.entries.slice(1).map((entry) => (
                    <div key={entry} className="memory-summary-item">{entry}</div>
                  ))}
                </div>
              )}
              <div className="flow-hub-actions">
            ...
            <article className="flow-hub-card">
              ...
              <div className="flow-hub-actions">
                <button className="hud-btn hud-btn-carbon hud-btn-mini" onClick={onOpenProfile}>
                  {t.flowOpenSettings}
                </button>
              </div>
            </article>
