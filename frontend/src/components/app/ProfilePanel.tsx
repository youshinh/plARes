...
const describeArStatus = (
  ...
  if (scanState === "ready")
    return `${t.arStatusReady ?? "Surface Ready"} (${scanPointCount})`;
  if (scanState === "tracking")
    return `${t.arStatusTracking ?? "Tracking"} (${scanPointCount})`;
  if (scanState === "searching")
    return t.arStatusSearching ?? "Scanning Surface";
  return scanState;
};
...
const describeRecentMatch = (
  ...
  const resultLabel =
    log.result === "WIN"
      ? (t.matchResultWin ?? "Win")
      : log.result === "LOSE"
        ? (t.matchResultLose ?? "Lose")
...
const formatAdkStatus = (status: string, t: UiText) => {
  ...
  if (status.startsWith("unavailable"))
    return t.liveUnavailable ?? "Unavailable";
  return status;
};
...
export const ProfilePanel: FC<ProfilePanelProps> = ({
  ...
  const arLabel = describeArStatus(
    isARSessionActive,
    scanState,
    scanPointCount,
    t,
  ...
  return (
    <>
      {!isProfileOpen && (
        <button
          type="button"
          className="hud-profile-launcher"
          onClick={onToggleProfile}
          aria-label={t.menu}
        >
          <span className="hud-profile-launcher-handle" />
          <span>{t.menu}</span>
        </button>
      )}
      <aside
        className={`hud-profile ${isProfileOpen ? "is-open" : ""} ${compactMatchMenu ? "is-match-mode" : ""}`}
      >
        ...
        {(!compactMatchMenu || matchTab === "settings") && (
          <>
            ...
            <div className="hud-profile-actions">
              <div
                className={`hud-align-pill ${alignmentReady ? "is-ready" : ""}`}
              >
              ...
              <label className="hud-lang-wrap hud-lang-editor">
                ...
                <div className="hud-lang-hint">
                  {inferLocaleLabel(selectedLanguage)}
                </div>
                <div className="hud-lang-hint">
                  {languageLocked
                    ? t.languageChangeAfterMatch
              ...
              <div className="hud-live-status-card">
                ...
                <div className="hud-live-status-grid">
                  ...
                  <strong>
                    {formatLiveRoute(liveDebugInfo.conversationRoute)}
                  </strong>
                  <span>{t.liveBattleRoute ?? "Battle Coach"}</span>
                  <strong>
                    {formatLiveRoute(liveDebugInfo.battleCoachingRoute)}
                  </strong>
                  <span>{t.liveCommentaryRoute ?? "Commentary"}</span>
                  <strong>
                    {formatLiveRoute(liveDebugInfo.commentaryRoute)}
                  </strong>
                  <span>{t.liveVisionRoute ?? "Vision Trigger"}</span>
                  <strong>
                    {formatLiveRoute(liveDebugInfo.visionTriggerRoute)}
                  </strong>
          ...
          <>
            ...
            <div className="hud-main-commands">
              ...
              <button
                ...
                onClick={
                  playMode === "match" ? onOpenBattlePrep : onOpenBattlePrep
                }
              >
                {t.prepTitle}
              </button>
              <button
                ...
                onClick={
                  playMode === "match" ? onReturnToHub : onOpenBattlePrep
                }
              >
              ...
              {!compactMatchMenu && (
                <>
                  <button
                    ...
                    onClick={
                      trainingSession ? onCompleteTraining : onStartTraining
                    }
                  >
        ...
        {!compactMatchMenu && (
          <>
            <div className="hud-profile-grid">
              ...
              <span>AR</span>
              <strong>{arLabel}</strong>
              <span>{t.matches}</span>
              <strong>{`${profileView.totalMatches} ${t.matchUnit ?? "matches"}`}</strong>
              <span>{t.training}</span>
              <strong>{`${profileView.totalTrainingSessions} ${t.sessionUnit ?? "sessions"}`}</strong>
              <span>{t.walks}</span>
              <strong>{`${profileView.totalWalkSessions} ${t.sessionUnit ?? "sessions"}`}</strong>
              <span>{t.tone}</span>
              <strong>{toneLabel}</strong>
              <span>{t.sync}</span>
              <strong>{`${Math.round(profileView.syncRate * 100)}%`}</strong>
              <span>{t.heat}</span>
        ...
        {!compactMatchMenu &&
          ...
          (liveDebugInfo.tokenName ||
            ...
            >
              ...
              {liveDebugInfo.lastStatus && (
                <div className="hud-truncate">{`Live Status: ${liveDebugInfo.lastStatus}`}</div>
              )}
              {liveDebugInfo.degradedReason && (
                <div className="hud-truncate">{`Degraded: ${liveDebugInfo.degradedReason}`}</div>
              )}
              {liveDebugInfo.tokenName && (
                <div className="hud-truncate">{`Token: ${liveDebugInfo.tokenName}`}</div>
              )}
              {liveDebugInfo.resumeHandle && (
                <div className="hud-truncate">{`Resume: ${liveDebugInfo.resumeHandle}`}</div>
              )}
              {liveDebugInfo.interactionId && (
                <div className="hud-truncate">{`Interaction: ${liveDebugInfo.interactionId}`}</div>
              )}
              {liveDebugInfo.interactionText && (
                <div className="hud-truncate">
                  {liveDebugInfo.interactionText}
                </div>
              )}
