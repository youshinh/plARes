...
import { resolveFallbackModelGlbPath, resolveModelGlbPath, resolveAnimationsGlbUrl, type ModelTypeId } from '../../constants/modelTypes';
...
export const useRobotAssetBundle = (modelType: ModelTypeId) => {
  ...
  const animationsUrl = resolveAnimationsGlbUrl(modelType);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        try {
          const animGltf = await loadAsync(animationsUrl);
          animationSource = animGltf.animations?.length ? animGltf.animations : animationSource;
        } catch (animErr) {
          console.warn(
            '[RobotCharacter] animation GLB load failed; using base clips',
            animationsUrl,
            animErr,
          );
        }
  ...
  }, [animationsUrl, fallbackModelUrl, heroModelUrl]);
