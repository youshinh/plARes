...
type UseRobotFrameLoopArgs = {
  ...
  attachments?: unknown; // To trigger cache re-population
};
...
export const useRobotFrameLoop = ({
  ...
  const cachedMaterialsRef = useRef<(THREE.MeshStandardMaterial | THREE.MeshPhysicalMaterial)[]>([]);

  // Re-populate cache when the model changes or the group is remounted
  useEffect(() => {
    ...
    group.traverse((child) => {
      if ((child as THREE.Mesh).isMesh) {
        materials.push((child as THREE.Mesh).material as THREE.MeshStandardMaterial | THREE.MeshPhysicalMaterial);
      }
    });
    cachedMaterialsRef.current = materials;
  }, [groupRef, modelGroupRef, attachments]);
  ...
  useFrame((_, delta) => {
    ...
    applyCombatGlow(cachedMaterialsRef.current, currentState, scarRoughnessBoost);
