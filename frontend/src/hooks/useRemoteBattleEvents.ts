...
import {
  normalizeArenaCalibration,
  useArenaSyncStore,
} from "../store/useArenaSyncStore";
...
type UseRemoteBattleEventsArgs = {
  ...
  setBgmFallbackCue: React.Dispatch<React.SetStateAction<number>>;
  ...
  setFusionCraftFlow: React.Dispatch<
    React.SetStateAction<FusionCraftFlowState>
  >;
...
const speakWinnerInterview = (text: string, lang: string) => {
  if (
  ...
  utterance.lang = lang || PLAYER_LANG;
  utterance.rate = 1.02;
  utterance.pitch = 1.0;
...
export const useRemoteBattleEvents = ({
  ...
  useEffect(() => {
    ...
    const handleRemoteBattleEvent = (event: GameEvent) => {
      ...
      if (payload && typeof payload === "object") {
        if (payload.kind === "arena_calibration") {
          ...
          const calibration = normalizeArenaCalibration(
            (payload as any).calibration,
          );
          if (!calibration) return;
          const arenaSync = useArenaSyncStore.getState();
          arenaSync.setRemoteCalibration(sender, calibration);
          const readyWithSender = useArenaSyncStore
            .getState()
            .hasAlignment(sender);
        ...
        if (
          ...
          const logsRaw = Array.isArray(profile.recent_match_logs)
            ? profile.recent_match_logs
            : [];
          ...
          if (stats && typeof stats === "object") {
            ...
            const material =
              rawMaterial === "Metal" || rawMaterial === "Resin"
                ? rawMaterial
                : "Wood";
            setRobotStats(
              {
          ...
          const recentAb = Array.isArray(profile.recent_dna_ab_tests)
        ...
        if (
          payload.kind === "dna_ab_feedback_saved" &&
          (!target || target === PLAYER_ID)
        ) {
          setRecentABFeedbackCount((count) =>
            Math.max(count, Number(payload.total ?? count)),
          );
          return;
        }
        if (
          payload.kind === "battle_status" &&
          (!target || target === PLAYER_ID)
        ) {
          const prev = battleStateRef.current;
          const maxHp = toPositiveNumber(payload.max_hp, prev.maxHp);
          const opponentMaxHp = toPositiveNumber(
            payload.opponent_max_hp,
            prev.opponentMaxHp,
          );
          const hpAfter = clampHp(toFiniteNumber(payload.hp, prev.hp), maxHp);
          const opponentHpAfter = clampHp(
            toFiniteNumber(payload.opponent_hp, prev.opponentHp),
            opponentMaxHp,
          );
        ...
        if (
          payload.kind === "special_not_ready" &&
          (!target || target === PLAYER_ID)
        ) {
        ...
        if (payload.kind === "damage_applied") {
          ...
          if (victim === PLAYER_ID) {
            ...
            setBattleState((prev) => {
              ...
              return {
                ...prev,
                opponentHp: clampHp(hpAfter, opponentMaxHp),
                opponentMaxHp,
              };
            });
          }
        ...
        if (
          payload.kind === "incantation_prompt" &&
        ...
        if (payload.kind === "bgm_ready") {
          ...
          if (url) {
            ...
            setBgmUrl("");
            setBgmFallbackCue((cue) => cue + 1);
            showSubtitle("Victory fanfare ready");
          }
          return;
        }
        ...
        if (payload.kind === "fused_item_error") {
          const message = String(
            payload.message ?? payload.error ?? "Fusion failed",
          );
          const requestId =
            typeof payload.request_id === "string" ? payload.request_id : "";
          showSubtitle(message);
        ...
        if (payload.kind === "match_pause") {
          ...
          showSubtitle(
            String(payload.message ?? "Match paused (connection issue)"),
          );
          return;
        }
        ...
        if (
          payload.kind === "live_ephemeral_token" &&
          (!target || target === PLAYER_ID)
        ) {
          if (payload.ok) {
            ...
            showSubtitle(
              `Live token ready (${String(payload.model ?? "model")})`,
            );
        ...
        if (
          payload.kind === "interaction_response" &&
          (!target || target === PLAYER_ID)
        ) {
          if (payload.ok) {
            const text = String(payload.text ?? "");
            setLiveDebugInfo((prev) => ({
              ...
              interactionId: String(
                payload.interaction_id ?? prev.interactionId ?? "",
              ),
        ...
        if (
          ...
          setLiveDebugInfo((prev) => ({
            ...
            degradedReason: available
              ? prev.degradedReason
              : detail || prev.degradedReason,
          }));
          return;
        }
        if (
          payload.kind === "battle_state_snapshot" &&
          (!target || target === PLAYER_ID)
        ) {
          ...
          showSubtitle(
            `Battle snapshot: HP ${hp}/${maxHp}, EX ${exGauge}, Sync ${Math.round(syncRate * 100)}%`,
          );
          return;
        }
        if (
          payload.kind === "tactical_recommendation" &&
          (!target || target === PLAYER_ID)
        ) {
      ...
      if (
        event.event === "winner_interview" &&
        payload &&
        typeof payload === "object"
      ) {
        const text = (payload as any).text;
        const lang = String((payload as any).lang ?? PLAYER_LANG);
        if (typeof text === "string" && text.trim()) {
          showSubtitle(text);
          speakWinnerInterview(text, lang);
        }
        return;
      }
      if (
        event.event === "proactive_line" &&
        payload &&
        typeof payload === "object"
      ) {
    ...
    const unsubscribe = wsService.addHandler(
      (payload: WebRTCDataChannelPayload) => {
